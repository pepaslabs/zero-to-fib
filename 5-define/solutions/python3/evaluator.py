...
def lisp_eval(ast, env):
    if ast["type"] == "number":
        ...
        # this list is a 'define' statement.
        if ast0["value"] == "define":
            if len(items) < 2:
                raise Exception("Missing parameter for 'define' statement")
            symbol_ast = items[1]
            symbol = symbol_ast["value"]
            if len(items) < 3:
                raise Exception("Missing argument for 'define' statement")
            value_ast = items[2]
            value = lisp_eval(value_ast, env)
            env[symbol] = value
            return None
