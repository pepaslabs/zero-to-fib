...
enum EvalutorError: Error {
    ...
    case missingParameter(_ list: [ASTNode])
    case missingParameterList(_ list: [ASTNode])
    case missingArguments(_ list: [ASTNode])
    case missingStatements(_ list: [ASTNode])
    case parametersMustBeAList(_ list: [ASTNode])
    case parametersMustBeSymbols(_ list: [ASTNode])
    case insufficientArgumentsForLambda(_ args: [LispValue])
    case evaluationMustProduceValue(_ expr: ASTNode)
}
...
func lisp_eval(ast: ASTNode, env: Environment) throws -> LispValue? {
    if let value = ast.number {
        ...
        // This is a "lambda" statement.
            ...
            if list.count < 2 {
                throw EvalutorError.missingParameter(list)
            }
            guard let param_list = list[1].list else {
                throw EvalutorError.parametersMustBeAList(list)
            }
            let params = try param_list.map({
                guard let symbol = $0.symbol else {
                    throw EvalutorError.parametersMustBeSymbols(list)
                }
                return symbol
            })
            if list.count < 3 {
                throw EvalutorError.missingStatements(list)
            }
        ...
        // This is a "define" statement.
        } else if list[0].symbol == "define" {
            if list.count < 2 {
                throw EvalutorError.missingParameterList(list)
            }
            guard let symbol = list[1].symbol else {
                throw EvalutorError.parametersMustBeSymbols(list)
            }
            if list.count < 3 {
                throw EvalutorError.missingStatements(list)
            ...
            env.values[symbol] = try lisp_eval(ast: statement, env: env)
            return nil
